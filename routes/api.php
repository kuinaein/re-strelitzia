...
Route::group(['middleware' => 'auth:api'], function (Router $router) {
    $router->group(['prefix' => 'account', 'namespace' => 'Account'], function (Router $router): void {
    ...
    $router->group(['prefix' => 'journal', 'namespace' => 'Journal'], function (Router $router): void {
    ...
    $router->resource('journal', 'Journal\JournalApiController', ['only' => ['store', 'update', 'destroy']]);
});
